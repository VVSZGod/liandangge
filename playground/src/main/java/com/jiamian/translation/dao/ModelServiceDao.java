...
public class ModelServiceDao {
	...
	public JSONObject getModelListByTag(Integer pageNo, Integer pageSize,
		...
		sql.append(
				" select m.id,m.model_id,m.name,m.type,m.create_date,m.description,m.downloadCount,m.rating,m.ldg_download_count,m.ali_url from  model m left join model_tags t on m.model_id=t.model_id"
		...
		if (SortTypeEnum.DOWN_COUNT.value().equals(sortType)) {
			sql.append(" order by (downloadCount+ldg_download_count) desc ,model_id desc");
